        ...
        try {
            ...
            const lastSales = await this.saleRepo.find({
                ...
                where: {
                    id: storeId,
                    created_at: Between(
                        new Date(lastMonthStart),
                        new Date(lastMonthEnd)
                    )
                }
            });
    ...
    async getInventoryKPIAnalytics(storeId: string) {
        try {
            ...
            for (const inv of inventories) {
                const category =
                    inv.product_variant.product.category_type ||
                    "Uncategorized";
        ...
        try {
            ...
            const avgSaleValue =
                thisWeekSales.length > 0
                    ? thisWeekRevenue / thisWeekSales.length
                    : 0;
            const lastAvgSaleValue =
                lastWeekSales.length > 0
                    ? lastWeekRevenue / lastWeekSales.length
                    : 0;

            const avgChange =
                lastAvgSaleValue === 0
                    ? 100
                    : ((avgSaleValue - lastAvgSaleValue) / lastAvgSaleValue) *
                      100;
    ...
    async getWeeklySalesTrend(storeId: string, weeks: number = 6) {
        try {
            ...
            const trendWithChange = sortedTrend.map((item, index) => {
                ...
                const revenueChange =
                    prev.totalRevenue > 0
                        ? ((item.totalRevenue - prev.totalRevenue) /
                              prev.totalRevenue) *
                          100
                        : null;
                ...
                const salesChange =
                    prev.totalSales > 0
                        ? ((item.totalSales - prev.totalSales) /
                              prev.totalSales) *
                          100
                        : null;
