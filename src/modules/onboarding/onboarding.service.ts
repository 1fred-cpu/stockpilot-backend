...
export class OnboardingService {
  ...
  private handleSupabaseError(operation: string, error: any) {
    if (error) {
      throw new BadRequestException(`Error ${operation}: ${error.message}`);
    }
  }
  ...
  async createStore(createStoreDto: CreateStoreDto, ownerId: string) {
    try {
      ...
      this.logger.error(`Error in createStore: ${error.message}`, error.stack);
  ...
  async createAdminUser(
    ...
    try {
      ...
      this.handleSupabaseError('checking admin user existence', existsError);
