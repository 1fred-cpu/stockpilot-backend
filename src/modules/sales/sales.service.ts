...
export class SalesService {
  ...
  constructor(
    ...
    private readonly recieptPrinterHelper: ReceiptPrinterHelper,
    ...
    try {
      return this.dataSource.transaction(async (manager) => {
        ...
        const sale = manager.create(Sale, {
          ...
          sale_code: generateReference('SALE'),
        ...
        if (dto.isRecieptNeeded) {
          ...
          } else if (dto.deliveryChannel === DeliveryChannel.PRINTER) {
            await this.handleRecieptOnPrinterDelivery(manager, sale.id);
          }
        }
        let receiptMessage = '';
  ...
  async findSale(saleCode: string) {
    try {
      const sale = await this.saleRepo.findOne({
        where: {
          sale_code: saleCode,
        },
        relations: ['saleItems', 'saleItems.productVariant'],
      });

      if (!sale) {
        throw new NotFoundException(
          `Cannot find sale created with this sale code ${saleCode}`,
        );
      }

      return sale;
    } catch (error) {
      this.errorHandler.handleServiceError(error, 'findSale');
    }
  }
  ...
  private async handleRecieptOnPrinterDelivery(
    manager: any,
    saleId: string,
    printerName?: string,
  ) {
    ...
    // Defines a receipt payload
    const reciept = await this.recieptHelper.generateReceipt(sale);

    // Generate a pdf of the receipt and get pdfUrl
    const pdfUrl = await this.recieptPdfHelper.savePdf(reciept, {
      ...sale.store,
      website: sale.business.website,
    });
    ...
    // Print PDF (auto fallback to default printer if not provided)
    const { error } = await this.recieptPrinterHelper.printPdf(
      pdfUrl,
      printerName,
    );
    if (error) {
  ...
  private async handleRecieptViaWhatsAppDelivery(manager: any, saleId: string) {
    // Fetch sale + items
    const sale = await manager.findOne(Sale, {
      where: { id: saleId },
      relations: ['saleItems', 'saleItems.productVariant', 'business', 'store'],
    ...
    // Save as PDF and get URL
    const pdfUrl = await this.recieptPdfHelper.savePdf(reciept, {
      ...sale.store,
      website: sale.business.website,
    });

    // Update sale record with pdf_url
    sale.pdf_url = pdfUrl;
    await manager.save(sale);

    // Send via WhatsApp
    await this.whatsappHelper.sendReceipt(
      sale.customer_phone,
      pdfUrl,
      sale.store.name,
    );
    ...
    return {
      message: `Receipt sent to WhatsApp number ${sale.customer_phone}`,
      saleId: sale.id,
      pdfUrl,
    };
  }
    ...
    try {
      ...
      const saleItems = sales.flatMap((sale) =>
        sale.saleItems.map((item) => ({
          ...
          status: item.status,
          returnQuantity: item.returns?.[0]?.quantity,
