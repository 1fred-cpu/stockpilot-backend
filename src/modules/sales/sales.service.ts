...
export class SalesService {
            ...
            return this.dataSource.transaction(async manager => {
                ...
                if (dto.isRecieptNeeded) {
                    if (dto.deliveryChannel === DeliveryChannel.EMAIL) {
                        await this.handleRecieptOnEmailDelivery(
                            manager,
                            sale.id
                        );
                    } else if (
                        dto.deliveryChannel === DeliveryChannel.PRINTER
                    ) {
                        await this.handleRecieptOnPrinterDelivery(
                            manager,
                            sale.id
                        );
                    }
    ...
    private async handleRecieptViaWhatsAppDelivery(
        manager: any,
        saleId: string
    ) {
        // Fetch sale + items
    ...
    private async deductStockTransactional(manager: any, dto: DeductStockDto) {
        try {
            ...
            this.logger.error(
                `Error in deductStockTransactional: ${error.message}`
            );
            throw error;
        }
    }
