...
export class ProductsService {
    ...
    async updateProduct(productId: string, dto: UpdateProductDto) {
        try {
            ...
            if (dto.thumbnail) {
                dto.thumbnail = await this.handleThumbnail(
                    product,
                    dto.thumbnail
                );
            }
    ...
    async updateProductVariant(
        ...
        if (updateVariantDto.reserved || updateVariantDto.lowStockThreshold) {
            ...
            const { reserved, lowStockThreshold } =
                varinatPlusInventory.inventories;
            ...
            const {
                ...
                .from("inventories")
                .update({
                    reserved: updateVariantDto.reserved ?? reserved,
                    lowStockThreshold:
                        updateVariantDto.lowStockThreshold ?? lowStockThreshold
                })
                .eq("variantId", updatedVariant.id);
    ...
    async getProductVariant(productId: string, variantId) {
        try {
            this.validateUUID(productId, "product ID");
            const product = await this.getProductById(productId);
    ...
    private async handleVariants(
        ...
        if (dto.variants) {
            for (const variant of dto.variants) {
                if (variant.id) {
                    await this.updateOrReplaceVariant(
                        product,
                        productId,
                        variant
                    );
