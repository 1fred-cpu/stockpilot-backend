...
export class ProductsService {
  ...
  async createProduct(createProductDto: any) {
    ...
    try {
      ...
      const path = `stores/${storeName}/${new Date().getTime()}_${thumbnail.originalname}`;
      ...
      const variantsToInsert = variants.map(async (variant) => {
        ...
        const path = `stores/${storeName}/${new Date().getTime()}_${variant.imageFile.originalname}`;
  ...
  async addProductVariant(
    ...
    try {
      ...
      // Define a path to upload file
      const path = `stores/${product.stores.storeName}/${new Date().getTime()}_${file.originalname}`;
