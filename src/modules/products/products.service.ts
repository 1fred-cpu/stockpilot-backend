...
export class ProductsService {
  ...
  async createProduct(createProductDto: any) {
    try {
      ...
      if (categoryError) {
        throw new BadRequestException(
          'Supabase Error: ' + categoryError.message,
        );
      }
  ...
  /* if exists and want to change image url
        // delete the previous image 
        // upload new image 
        // update image url

    if exists and want to update inventories
        // use inventoryId from variant to update inventories
  ...
  */
  ...
  async updateProduct(productId: string, updateProductDto: UpdateProductDto) {
    try {
      ...
      // Remove previous thumbnail file from bucket if want to change
      if (updateProductDto.thumbnail) {
        const previousThumbnailPath = getPathFromUrl(product.thumbnail);
        ...
        const path = `stores/${product.stores.storeName}/${new Date().getTime()}_${updateProductDto.thumbnail.originalname}`;
        const newThumbnailUrl = await this.fileUploadService.uploadFile(
      ...
      if (updateProductDto.variants) {
        ...
        for (const variant of updateProductDto.variants) {
          ...
          if (existingVariant) {
            // Check if variant image needs to be updated
            if (variant.imageFile) {
              const previousImagePath = getPathFromUrl(existingVariant.image);
              await this.fileUploadService.deleteFile(
                previousImagePath,
                'products',
              );
              ...
              const newImageUrl = await this.fileUploadService.uploadFile(
                variant.imageFile,
                path,
                'products',
              );
              variant.imageFile = newImageUrl;
            }

            // Update existing variant
            const { data, error } = await this.updateProductVariant(
              productId,
              variant.id,
              variant,
            );
            ...
            if (error) {
              throw new BadRequestException(
                'Supabase Error updating product variant: ' + error.message,
              );
            }
          } else {
