...
export class ProductsService {
    ...
    async createProduct(
        ...
        files: { thumbnail: Multer.File[]; variantImages: Multer.File[] }
    ) {
        try {
            const {
                storeId,
            ...
            const path = `stores/${storeName}/${Date.now}_${files.thumbnail[0].originalname}`;
            ...
            if (!thumbnail) {
                throw new BadRequestException("Thumbail file is required");
            }
            ...
            // Create variants
            const variantsToInsert = variants.map(async (variant, index) => {
                // Gets each vafiant image file with index
                const file = files.variantImages[index];
                // Define a path to upload file
                const path = `stores/${storeName}/${Date.now}_${file.originalname}`;
                // Bucket name to store files
                const bucket = "products";
                // Upload file and return url
                const imageUrl = await uploadFile(file, path, bucket);
    ...
    async addProductVariant(
        ...
        try {
            ...
            // Define a path to upload file
            const path = `stores/${product.stores.storeName}/${Date.now}_${file.originalname}`;
