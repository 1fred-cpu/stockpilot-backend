...
export class ProductsService {
    ...
    async deleteProduct(productId: string) {
        try {
            ...
            const product = deletedProduct || deletedProduct[0];
    ...
    async deleteProductVariant(productId: string, variantId: string) {
        ...
        if (!deletedVariant || !deletedVariant.length) {
            throw new NotFoundException("Product variant does not exist");
        }

        const variant = deletedVariant || deletedVariant[0];
        // Remove variant image from bucket
        const path = getPathFromUrl(existingVariant.imageUrl);
        await this.fileUploadService.deleteFile(path, "products");

        // return deletedVariant
        return deletedVariant[0] || deletedVariant;
    }
