    ...
    async findAllProductsByBusiness(businessId: string) {
        try {
            const products = await this.productRepo.find({
                ...
                relations: [
                    "productVariants",
                    "productVariants.storeInventories"
                ],
                select: {
                    id: true,
                    name: true,
                ...
                for (const variant of updateDto.productVariants) {
                    if (variant.id) {
                        ...
                        const inventoryPayload = {
                            ...
                            low_stock_quantity:
                                variant?.inventory?.lowStockQuantity
                        };
                        await manager.update(
                            StoreInventory,
                        ...
                        newFiles.push(
                            getPathFromUrl(createdVariant.qrcode_url)
                        );
                        ...
                        const inventory = manager.create(StoreInventory, {
                            ...
                            low_stock_quantity:
                                variant.inventory?.lowStockQuantity || 0
                        });
                    ...
                    if (product.productVariants?.length) {
                        for (const variant of product.productVariants) {
                            if (variant.image_url) {
                                ...
                                imagePaths.push(
                                    getPathFromUrl(variant.qrcode_url)
                                );
                            }
                        }
                    }
