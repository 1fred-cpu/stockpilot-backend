...
export class ProductsService {
    ...
    async updateProductVariant(
        ...
        try {
            ...
            const { data: productExists, error: checkError } =
                await this.supabase
                    .from("products")
                    .select("*")
                    .match({ id: productId })
            ...
            if (updateError) {
                throw new Error(
                    "An error occured while updating product variant"
                );
            }
    ...
    async addProductVariant(productId: string, variant: Variant) {
        try {
            ...
            const { data: productVariant, error: createError } =
                await this.supabase
                    .from("products variants")
                    .insert([newVariant])
                    .select();
    ...
    async getProductVariants(productId: string) {
        try {
            ...
            const { data: productVariants, error: fetchError } =
                await this.supabase
                    .from("products variants")
                    .select("*")
