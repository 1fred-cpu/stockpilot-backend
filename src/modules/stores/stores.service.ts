...
export class StoresService {
    ...
    try {
      ...
      const results = await Promise.all(
        stores.map(async (store) => {
          // ---- Get today's sales using saleRepo ----
          const salesData = await this.saleRepo
            .createQueryBuilder('sale')
            .leftJoin('sale.saleItems', 'saleItem')
            .select('SUM(DISTINCT sale.net_amount)', 'totalRevenue') // distinct ensures no double-counting
    ...
    try {
      return await this.dataSource.transaction(async (manager) => {
        ...
        const existingInvite = await manager.findOne(Invite, {
          where: { email: dto.email, store_id: storeId },
        });
        if (
          existingInvite &&
          new Date().getTime() < new Date(existingInvite.expires_at).getTime()
        ) {
          throw new ConflictException(
            `An invite has been already sent to this email. It will expire at ${new Date(
              existingInvite.expires_at,
            ).toLocaleDateString()}`,
          );
        }

        // 3. Check if user already in store
        ...
        // 4. Create invite record with expiry
        ...
        //   <p>This link will expire in ${new Date(
        //       invite.expires_at
        ...
        const { data, error } = await this.mailService.sendMail(
          ...
          `
                  <p>Hello,</p>
                  <p>You were invited to join <strong>${
                    store.name
                  }</strong>.</p>
                  <p>Click below to accept the invite:</p>
                  <p><a href="${inviteUrl}">Accept Invite</a></p>
  ...
  async acceptInvite(
    token: string,
    dto: { name: string; password: string; email: string },
  ) {
    // Will be used for cleanup of user creation in auth if error occure
    ...
    try {
      ...
      return await this.dataSource.transaction(async (manager) => {
        ...
        const inviteId = (
          await this.jwtHelper.verifyToken<{
            inviteId: string;
          }>(token)
        ).inviteId; // JWT decode
        ...
        if (!user) {
          ...
          // Store user id for cleanup if process failed
        ...
        const { error } = await this.mailService.sendMail(
          invite.email,
          `Your invitation to join ${store.name} was accepted`,
          `
                              <p>Hello ${dto.name},</p>
                              <p>Your invitation to join <strong>${store.name}</strong> has been accepted and your account is now active.</p>
                              <p>You can now log in and start using the platform.</p>
      ...
      // Check if user created in supabase auth and proceed cleanup
      if (createdAuthUserId) {
        const { data, error } =
          await this.supabase.auth.admin.deleteUser(createdAuthUserId);
        if (error) {
          console.error(error);
        }
      }
  ...
  async updateStoreUsers(storeId: string, dto: UpdateStoreUsersDto) {
    try {
      return await this.dataSource.transaction(async (manager) => {
        ...
        // Fetch updated users
        const storeUsers = await manager.find(StoreUser, {
          where: { store_id: storeId },
          relations: ['user'],
        });

        return {
          message: 'Store users updated successfully',
