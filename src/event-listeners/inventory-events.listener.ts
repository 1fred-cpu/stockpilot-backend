...
export class InventoryEventsListener {
    ...
    async handleInventoryEvents(message: any) {
        ...
        try {
            ...
            if (event === "InventoryDeducted") {
                await this.handleInventoryDeductedEvent(data);
            }
        } catch (error) {
    ...
    private async handleInventoryRestockEvent(data: any) {
        try {
            ...
            const { data: variantDetails, error: variantError } =
                await this.supabase
      ...
      `
            ...
            if (variantError)
                throw new BadRequestException(variantError.message);
          ...
          <tr>
            <td style="padding:8px;border:1px solid #ddd;">
              <img src="${
                  detail?.image_url || ""
              }" alt="variant image" style="max-width:50px; max-height:50px;"/>
            </td>
            <td style="padding:8px;border:1px solid #ddd;">
              <strong>${detail?.product.name || "Unknown Product"}</strong><br/>
    ...
    async handleInventoryStockAlertEvent(payload: any) {
        try {
            ...
            const details = await this.getProductVariantDetails(
                payload.variant_id
            );
